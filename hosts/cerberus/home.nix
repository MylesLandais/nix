...
in
{
  ...
  # Icon is installed into the hicolor theme (below) so launchers that only
  # resolve icons by theme name (not absolute paths) still find it.
  home.file.".local/share/icons/hicolor/512x512/apps/hermes-desktop.png".source =
    "${inputs.hermes-agent.packages.${pkgs.system}.desktop}/share/hermes-desktop/dist/hermes.png";
  ...
  # Custom Hermes dashboard theme matching the system's Stylix/Kanagawa
  # Dragon base16 scheme (home.nix stylix.base16Scheme). Hermes reads
  # user themes from ~/.hermes/dashboard-themes/*.yaml at dashboard load
  # (hermes_cli/web_server.py:_discover_user_themes) -- not part of the
  # unmanaged config.yaml, so this file is safe to own via Nix.
