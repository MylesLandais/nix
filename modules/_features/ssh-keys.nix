{ config, lib, ... }:
{
  # Setting authorizedKeys alone creates a partial user; mark warby as a
  # normal user by default so hosts that only import this module still
  # evaluate. Hosts that fully define warby (cerberus, lacie) override the
  # rest of the account.
  users.users.warby = {
